...
结论：Starlette 专项修复版本 `dd94924` 已将 FastAPI 升至 0.141.1、Starlette 升至 1.3.1，并增加解析前请求体限制。完整测试 91 项通过，依赖审计覆盖 49 个包，未发现已知漏洞。此结论仅针对此次依赖审计范围，不表示系统不存在其他漏洞；现有 EBS 加密迁移仍未完成。

## Starlette 修复复测

- 修复原扫描涉及的 5 个不同 CVE；原 10 条记录含重复项。
- 新增 ASGI 请求体限制，在 FastAPI 表单解析前执行；校验声明长度并累计实际接收字节，缺失或虚报 Content-Length 也不能绕过。上限为配置的 5MB 文件限制加 64KB 表单封装余量，原文件大小检查继续生效。
- 普通表单超过 1000 个字段或单字段超过 1MB 返回 400；整体请求超限返回 413。
- 适配 Starlette 1.x 的 TemplateResponse 请求参数，并补齐审计页的 CSRF 模板函数。
- 测试覆盖：未登录表单解析限制、无长度/虚报长度的分块请求、正常请求体回放、用户列表/批量上传/审计页面渲染；完整套件 91 项通过，`pip check` 无依赖冲突。
- `pip-audit -r requirements.txt --format json`：49 个包、0 条已知漏洞。
- 线上实例 `i-05f125f400c51017b` 已部署 `dd94924`，容器内确认 FastAPI 0.141.1 / Starlette 1.3.1；健康检查正常，1001 字段的小规模表单验证返回 400，登录入口返回 302。没有执行真实用户创建/删除操作或资源耗尽测试，完整 OIDC 登录回调未在本次自动验证中覆盖。

以下专项复核与早期部署记录为修复前的发现及处理过程。
...
| 优先级 | 问题 | 状态 |
|---|---|---|
| P0 | 依赖存在已知漏洞，包含 `authlib`、`starlette`、`python-multipart`、`jinja2` 等 | 联合升级已完成；当前依赖审计无已知漏洞 |
